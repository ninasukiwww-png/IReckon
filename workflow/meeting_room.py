...
class MessageLayer(Enum):
    L1_PUBLIC="L1"; L2_MEETING="L2"; L3_PRIVATE="L3"
...
class MeetingRoom:
    ...
    def __init__(self, task_id):
        self.task_id=task_id; self.room_id=f"room-{task_id}"; self.members={}
        self.history={layer:[] for layer in MessageLayer}
        self._private_queues={}; self._broadcast_queues={MessageLayer.L1_PUBLIC:[], MessageLayer.L2_MEETING:[]}
    ...
    async def broadcast(self, layer, sender_role, sender_id, content, msg_type="text", metadata=None, persist=True):
        ...
        async with self._lock:
            ...
            if len(self.history[layer])>self.MAX_HISTORY: self.history[layer]=self.history[layer][-self.MAX_HISTORY:]
        if persist: await self._persist(msg)
        ...
        for q in self._broadcast_queues.get(layer,[]): await q.put(msg)
        return msg
    ...
    async def send_private(self, sender_role, sender_id, recipient_role, recipient_id, content, msg_type="tool_request", metadata=None, persist=True):
        ...
        async with self._lock:
            ...
            if len(self.history[MessageLayer.L3_PRIVATE])>self.MAX_HISTORY: self.history[MessageLayer.L3_PRIVATE]=self.history[MessageLayer.L3_PRIVATE][-self.MAX_HISTORY:]
        if persist: await self._persist(msg)
        ...
        # WebSocket 推送私聊
        try:
            from web.websocket import push_message_to_websocket
            await push_message_to_websocket(self.task_id, {
                "msg_id": msg.msg_id, "layer": msg.layer.value, "sender_role": msg.sender_role, "sender_id": msg.sender_id,
                "content": msg.content, "msg_type": msg.msg_type, "metadata": {**msg.metadata, "recipient_role":recipient_role, "recipient_id":recipient_id},
                "timestamp": msg.timestamp.isoformat(),
            })
        except ImportError: pass
        except Exception as e: logger.warning(f"WebSocket 私聊推送失败: {e}")
        return msg

    async def _persist(self, msg):
        await db.execute("INSERT INTO conversation_messages(msg_id,task_id,layer,sender_role,sender_id,content,metadata,timestamp) VALUES(?,?,?,?,?,?,?,?)",
...
class MeetingRoomManager:
    ...
    def __init__(self):
        ...
        self._init=True; self._rooms={}; self._lock=asyncio.Lock()
    async def create_room(self, tid):
        async with self._lock:
            if tid in self._rooms: return self._rooms[tid]
            room = MeetingRoom(tid); self._rooms[tid]=room; logger.info(f"创建会议室: {room.room_id}"); return room
    async def get_room(self, tid): return self._rooms.get(tid)
    async def close_room(self, tid):
        async with self._lock:
            if tid in self._rooms: del self._rooms[tid]; logger.info(f"关闭会议室: room-{tid}")
